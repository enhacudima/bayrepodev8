...
@include('blacklist.inc.messages')
        ...
        <div class="container">
                <h4 class="text-left">
                    Upload List for Verification
                </h4>       
        

        {!!Form::open(['action'=>'ChecksController@upload','method'=>'POST','enctype'=>'multipart/form-data'])!!}
        ...
        <table id="table" name="table" class="table-responsive table-bordered table-hover " cellspacing="0" width="100%" style="font-size:12px">
            ...
            <tbody>
                ...
                @foreach ($checks->unique('identificador_de_bulk') as $check)
                <tr>                       
                    <td>{{$check->id}}</td>
                    <td>{{$check->identificador_de_bulk}}</td>
                    <td>{{$check->created_at}}</td>
                    <td>{{$check->name}} {{$check->lname}}</td>
                    <td><a href="{{url('')}}/download/{{$check->identificador_de_bulk}}" class=""><i class="btn btn-info fa fa-download btn-bg" aria-hidden="true"></i></a></td>
